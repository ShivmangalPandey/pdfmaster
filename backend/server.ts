...
const BLOGS_FILE = path.join(BACKEND_DIR, "blogs.json");
...
// Ensure local directories and files exist for fallback
const initDirs = () => {
...
// Database Helper with singleton pattern and retry logic
const connectDB = async (retries = 3): Promise<Db | null> => {
  if (db) return db;
  if (!MONGODB_URI) {
  ...
  while (retries > 0) {
    try {
      if (!mongoClient) {
        console.log(`🔌 [DB] Connecting to MongoDB... (Attempts left: ${retries})`);
        
        // Extracting if it is a SRV connection
      ...
      if (isSslAlert80 || err.code === 'ERR_SSL_TLSV1_ALERT_INTERNAL_ERROR') {
        ...
        console.error("👉 This is usually caused by the MongoDB Atlas IP Access List.");
        console.error("🛠️  Solution: Go to MongoDB Atlas > Network Access and 'Allow Access From Anywhere' (0.0.0.0/0) for this project.");
        retries = 0; // Don't retry indefinitely on network rejection
      } else {
...
// Helper for blogs (MongoDB with Local Fallback)
...
const saveBlogs = async (blogData: any) => {
  const database = await connectDB();
  if (database) {
    try {
      if (Array.isArray(blogData)) {
        // Migration/Bulk save if needed, or just return
        return;
      }
...
async function startServer() {
  ...
  // Enable trust proxy for express-rate-limit to correctly identify client IPs behind proxies
  ...
  const upload = multer({ 
    ...
    fileFilter: (req, file, cb) => {
      ...
      if (allowedTypes.includes(ext)) {
        ...
        cb(new Error(`File type ${ext} is not supported. Please upload PDF or image files.`) as any);
      }
    }
  });

  // Nuclear JSON Hardening for API
  // This middleware ensures that anything going into /api is treated as an API request.
  ...
  // Upload endpoint
  app.post("/api/upload", asyncHandler(async (req: any, res: any) => {
    console.log("📥 Received upload request to /api/upload");
    ...
    upload.array("files")(req, res, (err) => {
      ...
      try {
        ...
        console.error("🔥 Error in /api/upload handler logic:", error);
  ...
  app.get("/sitemap.xml", async (req, res) => {
    try {
      ...
      const toolUrls = [
        "/merge-pdf", "/compress-pdf", "/pdf-to-jpg", "/jpg-to-pdf", 
        "/edit-pdf", "/pdf-to-word", "/watermark-pdf", "/rotate-pdf", "/page-numbers-pdf"
      ];
  ...
  // Helper to find file path by ID in UPLOADS_DIR
  const findUploadedFile = async (id: string) => {
  ...
  // JPG to PDF endpoint
  app.post("/api/tools/jpg-to-pdf", asyncHandler(async (req: any, res: any) => {
  ...
  // Watermark endpoint
  app.post("/api/tools/watermark", asyncHandler(async (req: any, res: any) => {
    try {
      ...
      console.log(`💧 [TOOL] Watermark: "${text}" for ${filePath}`);
      ...
      console.error("🔥 [TOOL] Watermark error:", error);
      res.status(500).json({ error: error.message || "Failed to add watermark" });
    }
  }));

  // Page Numbers endpoint
  app.post("/api/tools/page-numbers", asyncHandler(async (req: any, res: any) => {
    try {
      const { fileIds } = req.body;
      if (!fileIds || fileIds.length === 0) return res.status(400).json({ error: "File required" });
      ...
      console.log(`🔢 [TOOL] Page Numbers: Processing ${filePath}`);
      const pdfBytes = await fs.readFile(filePath);
      const pdfDoc = await PDFDocument.load(pdfBytes);
      const pages = pdfDoc.getPages();
      ...
      console.error("🔥 [TOOL] Page Numbers error:", error);
      res.status(500).json({ error: error.message || "Failed to add page numbers" });
    }
  }));

  // Helper to cleanup uploads after processing
  const cleanupUploads = async (fileIds: string[]) => {
    try {
      const files = await fs.readdir(UPLOADS_DIR);
      for (const id of fileIds) {
        const filename = files.find(f => f.startsWith(id));
        if (filename) {
          ...
          console.log(`🧹 [CLEANUP] Deleted upload: ${filename}`);
        }
      }
    } catch (err) {
  ...
  // Merge endpoint
  app.post("/api/tools/merge", asyncHandler(async (req: any, res: any) => {
    try {
      const { fileIds } = req.body;
      ...
      console.log(`📑 [TOOL] Merge: Processing ${fileIds.length} files`);
      ...
      console.error("🔥 [TOOL] Merge error:", error);
      res.status(500).json({ error: error.message || "Failed to merge PDF files" });
    }
  }));

  // Rotate endpoint
  app.post("/api/tools/rotate", asyncHandler(async (req: any, res: any) => {
    try {
      const { fileIds } = req.body;
      if (!fileIds || fileIds.length === 0) return res.status(400).json({ error: "File required" });
      ...
      console.log(`🔄 [TOOL] Rotate: Processing ${filePath}`);
      const pdfBytes = await fs.readFile(filePath);
      const pdfDoc = await PDFDocument.load(pdfBytes);
      const pages = pdfDoc.getPages();
      ...
      console.error("🔥 [TOOL] Rotate error:", error);
      res.status(500).json({ error: error.message || "Failed to rotate PDF" });
    }
  }));

  // Compress endpoint
  app.post("/api/tools/compress", asyncHandler(async (req: any, res: any) => {
    try {
      ...
      try {
        console.log(`🏗️ [TOOL] Compress: Attempting Ghostscript conversion...`);
      ...
      console.error("🔥 [TOOL] Compress error:", error);
      res.status(500).json({ error: error.message || "Failed to compress PDF" });
    }
  }));

  // PDF to Word endpoint
  app.post("/api/tools/pdf-to-word", asyncHandler(async (req: any, res: any) => {
    const { fileIds } = req.body;
    if (!fileIds || fileIds.length === 0) return res.status(400).json({ error: "File required" });
    ...
    console.log(`📝 Extracting text from PDF: ${sourcePath}`);
    const dataBuffer = await fs.readFile(sourcePath);
    
    try {
      const data = await pdfParse(dataBuffer);
      
      console.log(`📄 Creating Word document from extracted text (${data.text.length} chars)`);
  ...
  // Edit endpoint
  app.post("/api/tools/edit", asyncHandler(async (req: any, res: any) => {
    ...
    console.log(`📄 Reading PDF for edit: ${filePath}`);
    const pdfBytes = await fs.readFile(filePath);
    
    let pdfDoc;
    try {
      console.log("🏗️ Loading PDF into pdf-lib...");
      pdfDoc = await PDFDocument.load(pdfBytes, { ignoreEncryption: true });
    } catch (loadErr: any) {
      console.error("🔥 PDF Load crash:", loadErr);
    ...
    // Case 1: Interactive Annotations (New behavior)
    if (annotations && Array.isArray(annotations) && annotations.length > 0) {
      ...
      for (const ann of annotations) {
        ...
        // Frontend origin is Top-Left, Backend origin is Bottom-Left
        const xPos = (ann.x / 100) * width;
        const yPos = height - ((ann.y / 100) * height);
        
        const color = hexToRgb(ann.color || '#000000');

        console.log(`📍 Placing "${ann.text}" at ${xPos.toFixed(1)}, ${yPos.toFixed(1)} on page ${ann.pageIndex + 1}`);
    ...
    // Case 2: Legacy/Fallback Watermark (Old behavior)
  ...
  // PDF to JPG endpoint
  app.post("/api/tools/pdf-to-jpg", asyncHandler(async (req: any, res: any) => {
    const { fileIds } = req.body;
    if (!fileIds || fileIds.length === 0) return res.status(400).json({ error: "File required" });
    ...
    console.log(`🖼️ Converting ${sourcePath} to JPG images...`);

    // GS Command: Extract pages to JPG
    // %d in OutputFile handles multiple pages
    ...
    if (generatedFiles.length === 1) {
      // Single page, just return the JPG
  ...
  // Final Catch-All for /api that reached this point (API 404)
  // This matches /api, /api/, /api/anything
  app.use("/api", (req, res) => {
    console.warn(`🚫 API 404 Fallthrough: ${req.method} ${req.url}`);
    res.status(404).json({
      success: false,
  ...
  // This catches errors from any routes defined above
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    console.log(`🚀 Serving production build from: ${FRONTEND_DIST}`);
  ...
  // Global Absolute Error Handler (The very last thing in the stack)
  app.use((err: any, req: any, res: any, next: any) => {
    console.error("🔥 ABSOLUTE_FINAL_ERROR_HANDLER:", err);

    if (res.headersSent) {
      return next(err);
    }

    // Comprehensive check for API requests
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`🚀 PDFMaster Server is fully operational!`);
